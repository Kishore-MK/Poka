...
export function AgentCard({ id, owner, name, description, tokenURI, reputationScore = 0, validationCount = 0 }: AgentCardProps) {
  return (
    <Link href={`/agent/${id}`}>
      <motion.div
        whileHover={{ y: -5 }}
      ...
      >
        <div className="flex items-start justify-between mb-4">
          {tokenURI ? (
            <Image src={tokenURI} alt="Agent" width={600} height={600} className="w-12 h-12  rounded-full" />
          ) : (
